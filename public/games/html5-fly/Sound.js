...
{
   if (typeof window.g_soundDataMap === 'undefined')
   {
      window.g_soundDataMap = {};
   }

   var soundMap = window.g_soundDataMap;

   if ( soundMap[name] == undefined )
   {
      dbg("Sound asset missing: " + name, false);
      this.myName = name;
      this.myInstanceArray = [];
      this.myNumInstances = 0;
      this.myLastInstancePlayed = null;
      this.myInstanceIndex = 0;
      this.play = function(){};
      this.playChr = function(){};
      return;
   }
   ...
   for ( var i = 0; i < this.myNumInstances; ++i )
   {
      ...
      this.myInstanceArray[i].src = soundMap[name];  // base64 encoded ogg
