...
const { loadDotEnvFiles, ensureDataDir } = require("./lib/outreach.cjs");
const {
  GMAIL_SESSION_PATH,
  buildAuthUrl,
  clearSession,
  ...
  validateRefreshEnv,
  waitForOAuthCode,
} = require("./lib/gmail.cjs");

function usage() {
  console.log(`
Usage:
  node scripts/outreach/gmail-oauth.cjs auth-url
  node scripts/outreach/gmail-oauth.cjs listen
  node scripts/outreach/gmail-oauth.cjs exchange-code --code <auth-code>
  node scripts/outreach/gmail-oauth.cjs test-profile
  node scripts/outreach/gmail-oauth.cjs send-test --to <email> [--subject "..."] [--body "..."]
